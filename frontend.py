...
class YOLO(object):
    ...
    def train(self, train_imgs,     # the list of images to train the model
        ...
        train, test = train_test_split(train_imgs, random_state=42, test_size=0.33)
        train_generator = BatchGenerator(train, 
                                     generator_config, 
                                     norm=self.feature_extractor.normalize)
        valid_generator = BatchGenerator(test, 
                                     generator_config, 
                                     norm=self.feature_extractor.normalize,
                                     jitter=False)   
        ...
        class TensorBoardImage(keras.callbacks.Callback):
            ...
            def on_epoch_begin(self, epoch, logs={}):
                ...
                for file in image_files:
                    ...
                    images.append(tf.Summary.Value(tag=file, image=image))
                summary = tf.Summary(value=images)
        ...
        self.model.fit_generator(generator        = train_generator, 
                                 ...
                                 callbacks        = [early_stop, checkpoint, tensorboard, tbi_callback], 
                                 workers          = 3,
                                 max_queue_size   = 8)      
